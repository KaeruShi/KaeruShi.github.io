...
export function SpotlightBackground({
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      spotlightsRef.current = spotlightsRef.current.map((spotlight, i) => ({
        ...spotlight,
        targetX: e.clientX + (i - (colorArray.length - 1) / 2) * 30,
        targetY: e.clientY + (i - (colorArray.length - 1) / 2) * 20,
      }));
    };
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      spotlightsRef.current = spotlightsRef.current.map((spotlight, i) => ({
        ...spotlight,
        targetX: e.touches[0].clientX + (i - (colorArray.length - 1) / 2) * 30,
        targetY: e.touches[0].clientY + (i - (colorArray.length - 1) / 2) * 20,
      }));
    };
